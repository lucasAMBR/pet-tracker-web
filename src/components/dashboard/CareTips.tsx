...
import { Droplets, Dumbbell, Utensils, CalendarClock, Syringe, MapPin } from "lucide-react";

type TipTone = "blue" | "green" | "orange";
type TipProps = { title: string; description: string; icon: ReactNode; tone?: TipTone };

function Tip({ title, description, icon, tone = "blue" }: TipProps) {
  const light =
    tone === "blue" ? "bg-blue-50" : tone === "green" ? "bg-emerald-50" : "bg-orange-50";
  const dark =
    tone === "blue"
      ? "dark:bg-blue-900/20"
      : tone === "green"
...
const PillBtn = ({
  ...
  const dark = {
    ...
    green: "dark:bg-emerald-900/30 dark:text-emerald-200 dark:hover:bg-emerald-900/40",
    orange: "dark:bg-orange-900/30 dark:text-orange-200 dark:hover:bg-orange-900/40",
  } as const;
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Tip
            ...
            icon={<Dumbbell className="h-5 w-5 text-emerald-600 dark:text-emerald-400" />}
            tone="green"
          />
          <Tip
            title="Alimentação"
